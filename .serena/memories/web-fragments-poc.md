...
- first Vite base set to /first/ (first/vite.config.ts) so assets resolve under /first/.
...
- first keeps Vite base /first/ but serves SPA HTML for /second/ in dev via a Vite dev-server middleware plugin (first/vite.config.ts), so http://localhost:5174/second/ renders SecondFragment directly.
